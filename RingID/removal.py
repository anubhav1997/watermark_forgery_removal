...
from diffusers import DiffusionPipeline, UNet2DConditionModel, DDIMScheduler, DDIMInverseScheduler, AutoencoderKL, DPMSolverMultistepInverseScheduler, DPMSolverMultistepScheduler, StableDiffusionPipeline#, FluxTransformer2DModel, Transformer2DModel, FluxPipeline, PixArtSigmaPipeline
...
if model_id == "CompVis/stable-diffusion-v1-4"  or model_id == "stabilityai/stable-diffusion-2-base":
    ...
    pipe = InversableStableDiffusionPipeline.from_pretrained(model_id, torch_dtype=torch.float16) #vae=vae, unet=unet,  , 
...
while i < args.end_iter:
    ...
    # save_img(generated_image, f"generated_images/sd1/{i}.png")
    # i+=1
    # continue 
    ...
    if vae_optimization is not None:
        clean_img, adv_noise = pgd_attack_lamda(generated_image, clean_img, vae_optimization, lamda=lamda, alpha=alpha, iters=n_iters, cutoff=args.cutoff, delta=args.delta)
    else:
        clean_img, adv_noise = pgd_attack_lamda(generated_image, clean_img, vae, lamda=lamda, alpha=alpha, iters=n_iters, cutoff=args.cutoff, delta=args.delta)
